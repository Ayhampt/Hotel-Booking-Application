...
const extendRoomAvailability = async (): Promise<void> => {
  try {
    ...
    const roomCategoriesWithLatestDates =
      await roomRepository.findLatestAvailabilityPerCategory();
    ...
    for (const categoryData of roomCategoriesWithLatestDates as any[]) {
...
const extendCategoryAvailability = async (categoryData: {
  ...
  try {
    const { roomCategoryId, lastDate } = categoryData;

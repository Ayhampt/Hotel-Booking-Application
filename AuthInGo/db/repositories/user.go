...
type UserRepository interface {
	...
	UpdateIsVerified(id int64) error
}
...
func (u *UserRepositoryImpl) UpdateIsVerified(id int64) error {
	query := "UPDATE users SET is_verified = true WHERE id = ?"
	result, err := u.db.Exec(query, id)
	if err != nil {
		fmt.Println("Error updating user verification status:", err)
		return err
	}
	...
	if rowsAffected == 0 {
		fmt.Println("No rows were affected, user not updated")
		return nil
	}
	fmt.Println("User verification status updated successfully, rows affected:", rowsAffected)
	return nil
}
